...
models<-DMELM;
	...
	finaldmel<-list(DMELM,roc_noh_trib,pvalues_noh_trib,roc_dmel,pvalues_dmel,roc_trib,pvalues_trib)
	names(finaldmel)<-c("Dmel_models","roc_noh_trib","pvalues_noh_trib","roc_dmel","pvalues_dmel","roc_trib","pvalues_trib")
models<-TRIBM;
        roc_noh_trib<-lapply(models,function(x){ res<-predict(x, GeneEssentiality::noh_trib, type="prob");
                pROC::roc(GeneEssentiality::noh_trib$Class,res$E,direction=">") } );
        alle<-as.data.frame(roc_noh_trib[[1]]$predictor/roc_noh_trib[[1]]$predictor)
	...
	finaltrib<-list(TRIBM,roc_noh_trib,pvalues_noh_trib,roc_dmel,pvalues_dmel,roc_trib,pvalues_trib)
	names(finaltrib)<-c("Trib_models","roc_noh_trib","pvalues_noh_trib","roc_dmel","pvalues_dmel","roc_trib","pvalues_trib")
	names(finaltrib$pvalues_noh_trib)<-c("RF","XGBT")
	names(finaltrib$pvalues_trib)<-c("RF","XGBT")
	names(finaltrib$pvalues_dmel)<-c("RF","XGBT")
	names(finaltrib$roc_noh_trib)<-c("RF","XGBT")
	names(finaltrib$roc_dmel)<-c("RF","XGBT")
	names(finaltrib$roc_trib)<-c("RF","XGBT")

	names(finaldmel$pvalues_noh_trib)<-c("RF","XGBT")
	names(finaldmel$pvalues_trib)<-c("RF","XGBT")
	names(finaldmel$pvalues_dmel)<-c("RF","XGBT")
	names(finaldmel$roc_noh_trib)<-c("RF","XGBT")
	names(finaldmel$roc_dmel)<-c("RF","XGBT")
...
plot(roc_xg_dmelmd10,legend=F,color="black",auc.main=F, print.auc=T,print.auc.cex=size,add=T,col="red",print.auc.y=0.4)
pvalue<-roc.test(roc_xg_dmelmd10,roc_dmelm244)
text(x=0.315, y = 0.35, labels = paste0("P-value= ",signif(pvalue$p.value,digits=3)),cex=size)
legend(x="topright",c("RF","XGBT"),col=c(1,2),lty=1)
...
plot(roc_xg_tribmd10,legend=F,color="black",auc.main=F, print.auc=T,print.auc.cex=size,add=T,col="red",print.auc.y=0.4)
